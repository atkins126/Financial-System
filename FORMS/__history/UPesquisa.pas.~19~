...
begin
  ...
  case CBChave.ItemIndex of
    ...
    begin
      ...
      dm.cdsPesquisa.Close;
      dm.sqlPesquisa.CommandText := 'SELECT * FROM MOVIMENTO WHERE CADASTRO BETWEEN '''+MKInicio.Text+''' AND '''+MKFim.Text+'''';
      Showmessage(dm.sqlPesquisa.CommandText);
      dm.cdsPesquisa.Open;
    end;
  ...
  LblQtde.Caption:='Localizados:' +#13+ InttoStr(dm.cdsPesquisa.RecordCount);
  ...
  //Soma a Saida
  dm.cdsPesquisa.First;  //vai pro inicio da tabela (primeiro registro)
  while not dm.cdsPesquisa.Eof do  //Enquanto não for o final da tabela
  begin
    if dm.cdsPesquisaTIPO.AsString ='SAIDA' then //Se o campo TIPO da tabela cdsPesquisa for igual a 'SAIDA'  então...
    begin
      Saida := Saida + dm.cdsPesquisaVALOR.AsFloat;
    end;
